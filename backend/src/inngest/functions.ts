...
/**
 * 1. ADMIT PATIENT FUNCTION
 */
export const admitPatient = inngest.createFunction(
  { 
    id: "admit-patient", 
    triggers: [{ event: "patient/admitted" }] 
  },
  async ({ event, step }) => {
    ...
    const aiAssignment = await step.run("ai-triage", async () => {
      ...
      const doctorDataStr = data.doctors
        .map((d) => `ID: ${d._id.toString()}, Name: ${d.name}, Spec: ${d.specialization}, Dept: ${d.department}`)
        .join("\n");
      const nurseDataStr = data.nurses
        .map((n) => `ID: ${n._id.toString()}, Name: ${n.name}, Dept: ${n.department}`)
        .join("\n");
    ...
    const updatedPatient = await step.run("update-database", async () => {
      ...
      return await collection.findOne({ _id: new mongoose.Types.ObjectId(patientId) });
    });
...
export const analyzeXRayJob = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const updatedLab = await step.run("update-db", async () => {
      const updatedLabResult = await labResults.findByIdAndUpdate(
        labResultId,
        { aiAnalysis, status: "analyzed" },
        { new: true }
      ).lean();

      if (!updatedLabResult) throw new NonRetriableError("Lab result not found");
...
/**
 * 3. ADD CHARGE TO INVOICE FUNCTION
 */
export const addChargeToInvoice = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const inv = await step.run("handle-invoice", async () => {
      let existingInv = await invoice.findOne({ patientId, status: "draft" });
      if (!existingInv) {
        existingInv = new invoice({ patientId, items: [], totalAmount: 0 });
      }
      ...
      existingInv.totalAmount += priceInCents;
